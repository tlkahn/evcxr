...
;; Revisor: Yong Guo <guoyong206@gmail.com>
;; Created: 26 Sep 2018
;; Updated: Tue Feb 15 06:27:55 CST 2022
;; Version: 0.0.1
...
(defun evcxr-install-cargo (lst)
  (when lst
    (evcxr--add-dep (car lst))
    (evcxr-install-cargo (cdr lst))))

(defun evcxr--add-dep(dep-cell)
  (let* (
         (dep (car dep-cell))
         (version (cadr dep-cell))
         (dependency (concat ":dep " (format "%s" dep) "=" (format "'%s'" version)))
         )
    (progn
...
(defun evcxr-load-deps-from-toml-text ()
  (let* ((path (evcxr-get-cargo-file))
         (deps-str (shell-command-to-string (concat "y-cargo-deps " path)))
         (deps-str-lst (split-string deps-str "\n"))
         )
    (butlast (mapcar (lambda (dep-str)
              (split-string dep-str ":")
              )
            deps-str-lst))
    )
  )

(defun evcxr-load-deps-from-cargo()
  (interactive)
  (evcxr-install-cargo
   (evcxr-load-deps-from-toml-text)))
